...
export class RenteroLayoutComponent implements OnInit {
  ...
  async eliminarPropiedad(propiedadId: number): Promise<void> {
    const propiedad = this.propiedades.find(p => p.id === propiedadId);
    const nombrePropiedad = propiedad ? propiedad.nombre : `Propiedad ID ${propiedadId}`;
    ...
    if (confirmar) {
      this.alertasService.mostrarInfo(
        'Funcionalidad de eliminar propiedad pendiente de implementación',
        'En desarrollo'
      );
    }
  }
